...
> Simple Better Auth + MongoDB version. This project uses a direct `MONGODB_URI` like the provided working reference projects. See `SIMPLE_AUTH_SETUP_BN.md` for setup.
...
All livestock images are stored in `public/images` and referenced with `/images/...` paths. The app no longer depends on external Unsplash image URLs for animal cards or the hero image, so image 404 errors are avoided during local development and Vercel deployment.
